...
@section('noi_dung')
    ...
    $thanhTien = $detail->menuItem->Price * $detail->quantity_ordered;
                ...
                <h1>Vị trí của shipper  tuyến đường đến nhà hàng</h1>
        ...
        fetch(osrmUrl)
            ...
            <script>
                ...
                channel.bind('ShipperAcceptedOrder', function (data) {
                    //console.log('Shipper đã nhận đơn hàng:', data);
...
<head>
    <meta charset="UTF-8">
    <title>Track Shipper RealTime</title>
...
<body>
    ...
    <h2>🛵 Đang theo dõi Shipper di chuyển đến nhà hàng...</h2>

    <div id="map"></div>

    <script>
        var restaurantLat = {{ $restaurantLatitude }};
        var restaurantLng = {{ $restaurantLongitude }};
        var orderId = {{ $order->id }};

        var map = L.map('map').setView([restaurantLat, restaurantLng], 14);
        ...
        // Marker shipper (khởi tạo trống)
        var shipperMarker = null;
        var routeLine = null;

        function fetchShipperLocation() {
            fetch(`/api/get-shipper-location/${orderId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.latitude && data.longitude) {
                        ...
                        if (!shipperMarker) {
                            shipperMarker = L.marker([lat, lng], { icon: blueIcon }).addTo(map)
                                .bindPopup('Vị trí Shipper').openPopup();
                        } else {
                            shipperMarker.setLatLng([lat, lng]);
                        }

                        if (!routeLine) {
                            routeLine = L.polyline([[lat, lng], [restaurantLat, restaurantLng]], { color: 'blue' }).addTo(map);
                        } else {
                            routeLine.setLatLngs([[lat, lng], [restaurantLat, restaurantLng]]);
                        }
        ...
        // Cập nhật mỗi 5 giây
        setInterval(fetchShipperLocation, 5000);

        // Icon xanh cho shipper
        var blueIcon = new L.Icon({
            iconUrl: 'https://chart.googleapis.com/chart?chst=d_map_pin_icon&chld=truck|00f',
            iconSize: [40, 40],
            iconAnchor: [20, 40],
            popupAnchor: [0, -40]
        });
